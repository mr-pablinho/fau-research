...
def extract_post_convergence_parameters(dream_csv_path, convergence_point=None):
    ...
    param_cols = [
        col
        for col in dream_results.columns
    ...
    if not param_cols:
        ...
        non_param_cols = (
            ["like1"]
            + [col for col in dream_results.columns if col.startswith("simulation")]
            + ["chain"]
        )
    ...
    for col in param_cols:
        if col.startswith("par"):
            ...
            for param in CALIBRATE_PARAMS:
                if param.lower().replace("_", "") == clean_name.lower().replace(
                    "_", ""
                ):
...
def run_ensemble_models(full_param_sets, timestamp=None):
    ...
    for i, param_set in enumerate(full_param_sets):
        ...
        if len(full_param_sets) <= 10 or i % 10 == 0 or i == len(full_param_sets) - 1:
            print(
                f"   🔄 Running model {i+1}/{len(full_param_sets)} ({100*(i+1)/len(full_param_sets):.1f}%)"
            )
        ...
        try:
            ...
            model, model_ws = GWM(
                ...
                Kriv_Isar=param_set["Kriv_Isar"],
                Kriv_Muhlbach=param_set["Kriv_Muhlbach"],
                Kriv_Giessen=param_set["Kriv_Giessen"],
                Kriv_Griesbach=param_set["Kriv_Griesbach"],
                Kriv_Schwabinger_Bach=param_set["Kriv_Schwabinger_Bach"],
                Kriv_Wiesackerbach=param_set["Kriv_Wiesackerbach"],
                D_rch1=param_set["D_rch1"],
                D_rch2=param_set["D_rch2"],
            ...
            if os.path.exists(hds_path):
                ...
                for kstpkper in available_times:
                    ...
                    head_data[f"stress_period_{kstpkper[1]}"] = head_array[
                        0
                    ]  # Only layer 0
...
def create_probability_maps(
    ...
    if target_stress_periods is None:
        ...
        target_stress_periods = [
            sp for sp in target_stress_periods if sp in stress_periods
        ]
    ...
    for sp in target_stress_periods:
        ...
        for depth_threshold in depth_thresholds:
            ...
            for file_idx, head_file in enumerate(head_files):
                ...
                if (
                    file_idx % max(1, total_files // 10) == 0
                    or file_idx == total_files - 1
                ):
                    print(
                        f"      📊 Processing {sp} (depth {depth_threshold}m): {file_idx+1}/{total_files} ({100*(file_idx+1)/total_files:.1f}%)"
                    )
    ...
    for sp, sp_data in probability_maps.items():
        ...
        for i, depth_threshold in enumerate(depth_thresholds):
            ...
            if threshold_key in sp_data:
                ...
                im = axes[i].imshow(
                    probability, cmap=cmap, vmin=0, vmax=1, origin="upper"
                )
...
def create_summary_plots(probability_maps, plots_dir, depth_thresholds):
    ...
    for threshold_key, stats_df in all_stats_data.items():
        stats_csv_file = os.path.join(
            plots_dir, f"probability_statistics_{threshold_key}.csv"
        )
        stats_df.to_csv(stats_csv_file, index=False)

    print(
        f"   Summary plots and statistics saved for {len(depth_thresholds)} depth thresholds"
    )
...
def main():
    ...
    try:
        ...
        if max_runs is not None and max_runs > 0:
            ...
            print(
                f"🧪 TESTING MODE: Limited to {len(full_param_sets)} runs (out of {original_count} available)"
            )
        else:
            print(
        ...
        results_summary = run_ensemble_models(
            full_param_sets, timestamp=dream_timestamp
        )

        # Step 4: Create probability maps
        if results_summary["successful_runs"] > 0:
            probability_maps = create_probability_maps(
                results_summary,
